  ...
  $.fn.extend({ 
    aCollapTable: function(options) {
      ...
      var _collaptable = function($element, $parent, $display)
      {
        ...
        $('tr[data-parent='+parent+']', '.collaptable').each(function(key, item){
          $(item,self).css('display', display);
          if ($(item,self).hasClass('act-tr-expanded')) {
            _collaptable($element, $(item,self).data('id'), display);   
          }
        });

         $('tr[data-parent='+parent+']', '.collaptable2').each(function(key, item){
          $(item,self).css('display', display);
          if ($(item,self).hasClass('act-tr-expanded')) {
            _collaptable($element, $(item,self).data('id'), display);   
          }
      ...
      var _levelsAndParents = function(obj)
      {
        $('tr', obj).each(function(k, item){
          if ($(item,self).data('id')) {
            parent = { id : $(item,self).data('id'), parent : $(item,self).data('parent') };
            parents.push(parent);
          }
        });
        ...
        $('tr', obj).each(function(k, item){
          if ($(item,self).data('id')) {
            level = _getLevel($(item,self));
            $(item,self).attr('data-level', level);
          }
        });
      };
      ...
      var _bindButtons = function()
      {
        ...
        $(document).on('click', '.act-button-expand-all', function(){
          ...
          if ( $('tr','.collaptable').length > 0 ) {
            collapseds = [];
          $('tr','.collaptable').each(function(k, item){
              if ($(item,'.collaptable').hasClass('act-tr-collapsed')) {
                _collaptable($('.act-more', $(item,'.collaptable')));
            }
          });
          }
        });
        ...
        $(document).on('click', '.act-button-collapse-all', function(){
          ...
          if ( $('tr','.collaptable').length > 0 ) {
            collapseds = [];
            $('tr','.collaptable').each(function(k, item){
              if ($(item,'.collaptable').hasClass('act-tr-expanded')) {
                _collaptable($('.act-more', $(item,'.collaptable')));
              }
            });
          }
        ...
        $(document).on('click', '.act-button-expand-all2', function(){
          console.log('self3--->>'+self);
          if ( $('tr','.collaptable2').length > 0 ) {
            collapseds = [];
          $('tr', '.collaptable2').each(function(k, item){
              if ($(item,'.collaptable2').hasClass('act-tr-collapsed')) {
                _collaptable($('.act-more', $(item,'.collaptable2')));
            }
          });
          }
        });

        $(document).on('click', '.act-button-collapse-all2', function(){
          console.log('self4--->>'+self);
          if ( $('tr','.collaptable2').length > 0 ) {
            collapseds = [];
            $('tr', '.collaptable2').each(function(k, item){
              if ($(item,'.collaptable2').hasClass('act-tr-expanded')) {
                _collaptable($('.act-more', $(item,'.collaptable2')));
              }
            });
          }
